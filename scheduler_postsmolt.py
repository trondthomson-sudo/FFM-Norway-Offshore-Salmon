"""
scheduler_postsmolt.py - Post-smolt landanlegg: kohorter BAKOVER fra ABD-leveranse,
...
Cohort-dataklassen fra scheduler_1tank.py uendret. Det som er NYTT:
  ...
  1. PLASSERING: hver kohort får sin leveringsuke fra ABD-innsettplanen
     ...
     og settes inn VEKSTUKER før det. Ingen tankrotasjon - landanlegget har
...
cfg må ha (se config_postsmolt.py): N_ABD, ABD_INNSETT_MANEDER,
...
innsett på landanlegget) slik at ledgeren og appen bruker riktig ukeakse.
"""
...
def abd_leveringsdatoer(cfg) -> list[tuple[int, date]]:
    ...
    på dato. ABD-innsett = landanleggets leveranse (brønnbåt samme uke)."""
...
# Hovedfunksjon
# ----------------------------------------------------------------------
def build_postsmolt_schedule(cfg, growth_tables: GrowthTables | None = None):
    ...
    # ---- 2) Ukeakse: uke 0 = første innsett på landanlegget ----
    ...
    weekly_df = _assemble_wide_table_postsmolt(cfg, trinn, cohorts, generations, kapasitet, n_weeks_total)
...
# ----------------------------------------------------------------------
def _assemble_wide_table_postsmolt(cfg, trinn, cohorts, generations, kapasitet, n_weeks_total) -> pd.DataFrame:
    labels, dates = [], []
    for w in range(n_weeks_total):
...
if __name__ == "__main__":
    import config_postsmolt as cfg
    df, gens, cohorts, meta = build_postsmolt_schedule(cfg)
